...
public class FlightQueryHandler : IRequestHandler<MyFlightFormsQuery, List<MyFlightFormsResponseModel>>,
                                  ...
                                  IRequestHandler<GetMyFlightsQuery, List<MyFlightsResponseModel>>
{
    ...
    #region Lista de Fichas de Voo do Piloto
    ...
    #region Lista de Interessados em um Voo
    ...
    #region Pesquisa de Fichas de Voo por filtros
    public async Task<List<SearchFlightFormsResponseModel>> Handle(SearchFlightFormsQuery request, CancellationToken cancellationToken)
    {
        ...
        if (userId != Guid.Empty)
            query = query.Where(f => f.PilotId != userId);
    ...
    #region Busca Ficha de Voo por ID
    ...
    #region Retorna Status da Ficha de Voo
    ...
    #region Retorna voos do usuário autenticado
    public async Task<List<MyFlightsResponseModel>> Handle(GetMyFlightsQuery request, CancellationToken cancellationToken)
    {
        var user = await _unitOfWork.UserRepository.GetByIdAsync(request.UserId);
        if (user is null)
        {
            _notification.AddNotification("GetMyFlightsQuery", "Usuário informado não existe!");
        ...
        {
            var rating = await _unitOfWork.FlightRatingRepository.GetByFlightFormId(flight.Id);

            result.Add(new MyFlightsResponseModel
            {
                Id = flight.Id,
                DepartureDate = flight.DepartureDate,
                DepartureTime = flight.DepartureTime,
                DepartureAirport = flight.DepartureAirport,
                DepartureManualLocation = flight.DepartureManualLocation,
                ArrivalDate = flight.ArrivalDate,
                ArrivalTime = flight.ArrivalTime,
                ArrivalAirport = flight.ArrivalAirport,
                ArrivalManualLocation = flight.ArrivalManualLocation,
                AircraftType = flight.AircraftType,
                FlightComment = flight.FlightComment,
                HasOvernight = flight.HasOvernight,
                ...
                Pilot = new PilotResponseModel
                {
                    ...
                    Phone = flight.User.Phone
                },
                Rating = rating != null ? new RatingResponseModel
                {
